...
import static com.ericsson.oss.services.cm.importexportproxyservice.bulkimport.CmBulkImportServiceErrors.GET_UNSYNC_NODES_COUNT_JOB_STATUS_MISMATCH;
...
/**
 * Validation class for Unsynchronized Nodes Count in an Import Job.
 */
public class GetImportJobUnsyncNodesCountRequestValidator implements RequestValidator<GetImportJobUnsyncNodesCountRequest> {
    private static final List<JobStatus> INVALID_STATES_FOR_UNSYNC_NODES_COUNT = asList(CANCELLED, CREATED, PARSING, PARSED);

    @Inject
    ImportJobPersistenceService jobPersistence;

    @Override
    public List<ResponseMessage> validate(final GetImportJobUnsyncNodesCountRequest request) {
    ...
    private List<ResponseMessage> validateCurrentJobStatus(final ImportJob importJob) {
        if (INVALID_STATES_FOR_UNSYNC_NODES_COUNT.contains(importJob.getStatus())) {
            return asList(responseMessage()
                    .error(GET_UNSYNC_NODES_COUNT_JOB_STATUS_MISMATCH)
                    .build());
        }
        return Collections.emptyList();
    }
